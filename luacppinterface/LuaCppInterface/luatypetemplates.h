...
DEFINE_TYPE_TEMPLATE_FOR(int64_t, , lua_pushinteger(state.get(), (lua_Integer)param), int64_t res = (int64_t)lua_tointeger(state.get(), -1))
...
DEFINE_TYPE_TEMPLATE_FOR(uint64_t, , lua_pushunsigned(state.get(), (lua_Unsigned)param), uint64_t res = (uint64_t)lua_tounsigned(state.get(), -1))

DEFINE_TYPE_TEMPLATE_FOR(float,, lua_pushnumber(state.get(), param), float res = (float)lua_tonumber(state.get(), -1))
DEFINE_TYPE_TEMPLATE_FOR(double,, lua_pushnumber(state.get(), param), double res = (double)lua_tonumber(state.get(), -1))
